...
if (!supabaseUrl || !supabaseKey) {
  console.error('❌ ERRO: SUPABASE_URL e/ou SUPABASE_KEY não definidas.');
  process.exit(1);
}
...
function normalizarParentId(parent_id) {
  if (parent_id === null || parent_id === undefined || parent_id === '') {
    return null;
  }

  const numero = Number(parent_id);
...
async function validarHierarquia({ nome, nivel, parent_id, idAtual = null }) {
  ...
  const parentIdNumero = normalizarParentId(parent_id);
  ...
  if (Number.isNaN(parentIdNumero)) {
  ...
  if (nivelNumero === 1) {
    if (parentIdNumero !== null) {
      return {
        ok: false,
        status: 400,
        message: 'Registro de nível 1 (Grupo) não pode ter pai.'
      };
    }
  }
...
app.get('/', (req, res) => {
  res.send(`
    <html>
      <head>
        <meta charset="UTF-8" />
        <title>API Planejamento Financeiro</title>
      </head>
      <body style="font-family: Arial; padding: 20px;">
        <h1>API Planejamento Financeiro 🚀</h1>
        <p>Backend de categorias funcionando.</p>
      </body>
    </html>
  `);
});
...
app.get('/categorias/:id', async (req, res) => {
  try {
    ...
    if (error || !data) {
      if (error) {
        logErroSupabase(`GET /categorias/${id}`, error);
      }
    ...
    if (error) {
      ...
      if (error.message && error.message.toLowerCase().includes('duplicate')) {
        return res.status(400).json({
          error: 'Já existe uma categoria com esse nome dentro do mesmo pai.'
        });
      }
...
app.put('/categorias/:id', async (req, res) => {
  try {
    ...
    if (filhos && filhos.length > 0) {
      ...
      const algumFilhoInvalido = filhos.some(
        (filho) => Number(filho.nivel) !== filhosEsperados
      );
